...
const FALLBACK = [
  ...
  { _id: '2', title: 'Japan Tour Packages', duration: '6 Days / 5 Nights', description: "Discover Japan's beauty, culture, and traditions.", price: 760, originalPrice: 990, rating: 4.8, image: 'https://images.unsplash.com/photo-1493976040374-85c8e12f0c0e?w=500&auto=format&fit=crop' },
  { _id: '3', title: 'Vietnam Tour Packages', duration: '4 Days / 3 Nights', description: "Explore Vietnam's stunning landscapes and rich heritage.", price: 760, originalPrice: 990, rating: 4.8, image: 'https://images.unsplash.com/photo-1528360983277-13d401cdc186?w=500&auto=format&fit=crop' },
  { _id: '4', title: 'Switzerland Tour Packages', duration: '7 Days / 6 Nights', description: "Experience Switzerland's breathtaking beauty.", price: 760, originalPrice: 990, rating: 4.8, image: 'https://images.unsplash.com/photo-1531366936337-7c912a4589a7?w=500&auto=format&fit=crop' },
]
...
export default function PopularAdventures() {
  ...
  useEffect(() => { dispatch(fetchFeaturedPackages()) }, [dispatch])

  const packages = featured.length > 0 ? featured : FALLBACK
    ...
    <section ref={container} className="py-16 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="pa-grid grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {loading && packages === FALLBACK ? (
            [1,2,3,4].map((i) => <div key={i} className="pa-card rounded-2xl bg-gray-100 h-72 animate-pulse" />)
          ) : (
            packages.map((pkg) => (
